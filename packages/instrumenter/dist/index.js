...
export class CodeInstrumenter {
     ...
     */
    instrument(sourceCode, options = {}) {
        try {
            // Create source file in memory
            const sourceFile = this.project.createSourceFile(options.filePath || 'temp.ts', sourceCode, { overwrite: true });
            // Transform the code
     ...
     */
    collectArrayDeclarations(sourceFile) {
        const transformations = [];
    ...
    collectArraySwaps(sourceFile) {
        ...
        for (const expr of binaryExpressions) {
            ...
            if (operator.getKind() === SyntaxKind.EqualsToken) {
                ...
                if (left.getKind() === SyntaxKind.ArrayLiteralExpression &&
                    ...
                    if (leftArray && rightArray) {
                        ...
                        if (leftElements.length === 2 && rightElements.length === 2) {
                            ...
                            if (statement) {
                                ...
                                if (firstLeft?.getKind() === SyntaxKind.ElementAccessExpression &&
     ...
     */
    collectArrayAssignments(sourceFile) {
        const transformations = [];
        const binaryExpressions = sourceFile.getDescendantsOfKind(SyntaxKind.BinaryExpression);
        for (const expr of binaryExpressions) {
            const operator = expr.getOperatorToken();
...
export function instrument(sourceCode, options) {
